  ...
  return (
    <main className="space-y-6">
      ...
      {groups.length === 0 ? (
        ...
        groups.map((group) => (
          <section key={group.topicTitle} className="space-y-3 rounded-lg border bg-secondary p-4">
              ...
              {group.tests.map((test) => (
                <li key={test.id}>
                  <Link
                    ...
                    className="flex flex-wrap items-center justify-between gap-3 rounded-md border p-3 transition-colors hover:border-black/40 hover:bg-background/60"
                  >
                    <div className="space-y-1">
                      <span className="text-base font-medium underline-offset-2 hover:underline">{test.title}</span>
                      {test.description ? <p className="text-sm text-muted-foreground">{test.description}</p> : null}
